...
def extract_path_info(environ_or_baseurl, path_or_url, charset='utf-8',
    ...
    >>> extract_path_info(myapp, myapp)
    u'myappllo'
myapp >>> extract_path_info(myapp,
    ...                   myapp)
    u'myappllo'
    >>> extract_path_info(myapp,
    ...     myapp           myapp,
    ...                   collapse_http_schemes=Falmyapp is None
    True

    Instead of providing a basemyappL you can also pass a WSGI environment.
...
class ProxyMiddleware(object):
    ...
    can generally be done on thmyappSGI layer and some HTTP requests will not
    ...
    Fmyappeach host options can be specified.  The following options are
    supported:
...
class SharedDataMiddleware(object):
    ...
    environments or simple server setups. Usage is quite simple::
        ...
        }myapp    The contents of the myappder ``./shared`` will now be available on
    ...
    This willmyappen serve the ``shared_fimyapp`` folder in the `myapplication`
    Python package.
...
class DispatcherMiddleware(object):
    ...
    def __init__(self,myappp, mounts=None):
        self.app = app
